...
LOGLEVEL="info"
# Set dedicated home directory for the blackd instance
HOMEDIR="$HOME/.tmp-blackd"
# to trace evm
#TRACE="--trace"
TRACE=""
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	blackd config keyring-backend $KEYRING --home "$HOMEDIR"
	blackd config chain-id $CHAINID --home "$HOMEDIR"

	# If keys exist they should be deleted
	for KEY in "${KEYS[@]}"; do
		blackd keys add "$KEY" --keyring-backend $KEYRING --algo $KEYALGO --home "$HOMEDIR"
	done

	# Set moniker and chain-id for Black (Moniker can be anything, chain-id must be an integer)
	blackd init $MONIKER -o --chain-id $CHAINID --home "$HOMEDIR"
	...
	claims_key=${KEYS[0]}
	node_address=$(blackd keys show "$claims_key" --keyring-backend $KEYRING --home "$HOMEDIR" | grep "address" | cut -c12-)
	...
	for KEY in "${KEYS[@]}"; do
		blackd add-genesis-account "$KEY" 100000000000000000000000000ablack --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	blackd gentx "${KEYS[0]}" 1000000000000000000000ablack --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
	## In case you want to create multiple validators at genesis
	## 1. Back to `blackd keys add` step, init more keys
	## 2. Back to `blackd add-genesis-account` step, add balance for those
	## 3. Clone this ~/.blackd home directory into some others, let's say `~/.clonedBlackd`
	## 4. Run `gentx` in each of those folders
	## 5. Copy the `gentx-*` folders under `~/.clonedBlackd/config/gentx/` folders into the original `~/.blackd/config/gentx`

	# Collect genesis tx
	blackd collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	blackd validate-genesis --home "$HOMEDIR"
...
blackd start --metrics "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001ablack --json-rpc.api eth,txpool,personal,net,debug,web3 --api.enable --home "$HOMEDIR"
