...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	...
	Describe("from a non-authorized chain", func() {
		BeforeEach(func() {
			...
			err := s.GridironChain.App.(*app.Gridiron).ClaimsKeeper.SetParams(s.GridironChain.GetContext(), params)
		...
		It("should transfer and not recover tokens", func() {
			...
			nativeGridiron := s.GridironChain.App.(*app.Gridiron).BankKeeper.GetBalance(s.GridironChain.GetContext(), senderAcc, "afury")
		...
		Describe("to the sender's own eth_secp256k1 account on Gridiron (sender == recipient)", func() {
			...
			Context("with disabled recovery parameter", func() {
				BeforeEach(func() {
					...
					s.GridironChain.App.(*app.Gridiron).RecoveryKeeper.SetParams(s.GridironChain.GetContext(), params) //nolint:errcheck
				})
			...
			Context("with a sender's claims record", func() {
				...
				Context("with completed actions", func() {
					...
					It("should transfer tokens to the recipient and perform recovery", func() {
						...
						balanceEscrow := s.GridironChain.App.(*app.Gridiron).BankKeeper.GetBalance(s.GridironChain.GetContext(), transfertypes.GetEscrowAddress("transfer", "channel-0"), "afury")
						Expect(balanceEscrow).To(Equal(coinGridiron))
						ibcOsmo := s.GridironChain.App.(*app.Gridiron).BankKeeper.GetBalance(s.GridironChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
						Expect(ibcOsmo.IsZero()).To(BeTrue())

						// Relay both packets that were sent in the ibc_callback
						err := s.pathOsmosisGridiron.RelayPacket(CreatePacket("10000", "afury", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
						s.Require().NoError(err)
						...
						ibcGridiron := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), receiverAcc, teststypes.AfuryIbcdenom)
						Expect(ibcGridiron).To(Equal(sdk.NewCoin(teststypes.AfuryIbcdenom, coinGridiron.Amount)))
			...
			Context("without a sender's claims record", func() {
				...
				When("recipient has additional non-native ibc vouchers that originated from senders chains", func() {
					BeforeEach(func() {
						params := types.DefaultParams()
						params.EnableRecovery = false
						err := s.GridironChain.App.(*app.Gridiron).RecoveryKeeper.SetParams(s.GridironChain.GetContext(), params)
						s.Require().NoError(err)
