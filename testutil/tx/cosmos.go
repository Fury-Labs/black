...
var (
	...
	DefaultFee = sdk.NewCoin(utils.BaseDenom, sdk.NewIntFromUint64(uint64(feeAmt))) // 0.01 GRIDIRON
)
...
func signCosmosTx(
	...
	seq, err := appGridiron.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appGridiron.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
